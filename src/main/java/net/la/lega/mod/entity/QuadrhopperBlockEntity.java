...
public class QuadrhopperBlockEntity extends AInventoryEntity implements Hopper, Tickable, NamedScreenHandlerFactory, ExtendedScreenHandlerFactory
{
    ...
    {
        boolean ret = canAcceptItemType(stack.getItem(), slot);
        return ret;
    }
    ...
    private boolean canAcceptItemType(Item item, int slot)
    {
        for(int i = 0; i < items.size(); i++)
        {
            ...
            if(stack.getItem().equals(item))
            {
                if(slot == i)
                {
                    return stack.getCount() < getMaxCountPerStack();
                }
                else
                {
                    return false;
    ...
    private boolean insert()
    {
        ...
        else
        {
            ...
            {
                for(int i = lastUsedIndex; i < this.size(); i++)
                {
    ...
    {
        return new QuadrhopperBlockGUIHandler(syncId, inv, ScreenHandlerContext.create(world, pos));
    }
